<template>
  <div class="flex flex-col h-full w-full min-h-0 p-6 gap-4">
    <el-card shadow="never" class="search-card">
      <el-form :model="queryState" inline class="search-form">
        ...
        <el-form-item label="风险拨备策略">
          <el-select v-model="queryState.risk_strategy" clearable placeholder="全部" class="w-40">
            <el-option v-for="option in riskStrategyOptions" :key="option.value" :label="option.label" :value="option.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="状态">
          <el-select v-model="queryState.is_active" clearable placeholder="全部" class="w-28">
    ...
    >
      <div class="p-4 border-b border-gray-100 flex justify-between items-center bg-white flex-shrink-0">
        <div>
          ...
          <div class="text-xs text-gray-400 mt-1">用于统一管理净现值测算所需的生命周期、折现、风险拨备与残值规则。</div>
        </div>
        <el-button v-if="canWrite" type="primary" :icon="Plus" @click="openCreateDialog">新增基准</el-button>
      </div>

      <div class="flex-1 min-h-0 p-4">
        <el-table v-loading="loading" :data="baselineList" stripe height="100%" class="w-full">
          ...
          <el-table-column label="维保递增率" width="140" align="right">
            <template #default="{ row }">{{ formatPercent(row.corrosion_rate) }}</template>
          </el-table-column>
          <el-table-column label="风险拨备策略" width="140">
            <template #default="{ row }">
              <el-tag :type="row.risk_strategy === 'FIXED' ? 'success' : 'warning'" effect="plain">
          ...
          <el-table-column label="期末残值率" width="140" align="right">
            <template #default="{ row }">{{ formatPercent(row.eol_salvage_rate) }}</template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="160" fixed="right">
            <template #default="{ row }">
              <el-button v-if="canWrite" type="primary" link @click="openEditDialog(row)">编辑</el-button>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogMode === 'create' ? '新增 LCC 财务评估基准' : '编辑 LCC 财务评估基准'" width="760px" destroy-on-close>
      <el-form ref="formRef" :model="formState" :rules="formRules" label-width="130px" class="baseline-form">
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="规则名称" prop="rule_name">
              <el-input v-model="formState.rule_name" placeholder="例如：涉氯化工高危基准" maxlength="100" show-word-limit />
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="生命周期 (年)" prop="lifecycle_years">
              <div class="unit-input-row">
                <el-input-number v-model="formState.lifecycle_years" :min="1" :step="1" :precision="0" controls-position="right" class="unit-input-control" />
          ...
          <el-col :span="12">
            <el-form-item label="折现率 (WACC)" prop="discount_rate">
              <div class="unit-input-row">
                <el-input-number v-model="formState.discount_rate" :min="0" :step="0.1" :precision="4" controls-position="right" class="unit-input-control" />
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="设备维保递增率" prop="corrosion_rate">
              <div class="unit-input-row">
                <el-input-number v-model="formState.corrosion_rate" :min="0" :step="0.1" :precision="4" controls-position="right" class="unit-input-control" />
                <span class="unit-suffix">%</span>
              </div>
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="拨备数值" prop="risk_value">
              <div class="unit-input-row">
                <el-input-number v-model="formState.risk_value" :min="0" :step="0.1" :precision="4" controls-position="right" class="unit-input-control" />
          ...
          <el-col :span="12">
            <el-form-item label="期末处置残值率" prop="eol_salvage_rate">
              <div class="unit-input-row">
                <el-input-number v-model="formState.eol_salvage_rate" :step="0.1" :precision="4" controls-position="right" class="unit-input-control" />
                <span class="unit-suffix">%</span>
              </div>
        ...
        <el-form-item label="是否启用" prop="is_active">
          <el-switch v-model="formState.is_active" active-text="启用" inactive-text="禁用" />
        </el-form-item>
      </el-form>
